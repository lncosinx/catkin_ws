...
- **进阶模式（校验/落位器，D4 定稿）**：**外部下发一个有序的形状序列**，系统
  「照单放」。策略不再自由选形状，而是按外部给定顺序，为每个指定 shape 选择
  满足约束的目标格与旋转，并校验合法性（不合法则报错/跳过）。即进阶模式下策略
  是「给定形状序列 → 落位 + 校验」，不是自由求解器。
  - 外部序列来源：话题或参数（如 `/strategy/shape_sequence` 或 `~shape_seq`）。
  - 颜色↔形状绑定，指定形状即指定颜色，无需额外颜色识别。
...
| `/strategy/shape_sequence`（新，仅进阶） | 自定义/`Int32MultiArray` | 外部 → 策略 | 外部下发的有序形状序列，进阶模式照单放 |
...
- **D4 进阶任务语义**：**外部下发形状序列，系统照单放**。进阶模式下策略是
  「按外部形状序列落位 + 合法性校验」，非自由求解器。详见 §4。
