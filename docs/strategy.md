...
`strategy_node` 是流水线中负责"装箱决策"的核心。它接收视觉节点输出的库存与散料像素坐标，把"在 14×10 网格里尽可能多得分"当作一个**带计分的精确覆盖问题**求解，最终在 `/tetris_plan`（latched）上发布一份逐块的抓放方案，交给 `xarm_controller_node` 执行。
...
### 输出：`/tetris_plan`（`std_msgs/Int32MultiArray`，latched）
- 首元素：动作数 `N`。
- 之后每个动作 **17 个整数**：
  ```
  ...
  - `sum_r`/`sum_c`：该块 4 个格子行/列坐标的**绝对总和**（控制节点除以 4 得中心）。
...
### 形状 id 契约（与 vision / controller 共享，改动需同步全部）
...
- **带重启的随机化**：每个方案做 `max_restarts` 次，每次 `shuffle` 行顺序后重新建 DLX 搜索（`search` 受 `max_search_nodes` 与最大解数限制），对所有找到的解逐一计分取最优。
- **计分规则**（`solveForMask` 内）：
  - 满行 +10；满行且该行 **≥4 种颜色** 再 +10；
...
3. 按拓扑序为每个棋子从对应形状的散料里弹出一个像素抓取点（`available_blocks[shape].back()`），组装 17-int 动作并发布。无对应像素时告警并用 `(0,0)`。

发布后 `task_completed = true`，本次任务结束。
...
- 求解器 `solveSequence`（`tetris_solver.hpp`）：
  ...
  - 计分与普通模式一致（满行 +10、≥4 色再 +10、整盘形状种类 <3 则 0 分）。
- 输出沿用同样的 17-int 格式；按放置顺序直接打包（跳过拓扑排序），从散料弹出抓取像素。
...
手动喂入 `/vision/board_state` 触发规划，观察 `/tetris_plan`：
