...
# -*- coding: utf-8 -*-
"""
Enhanced calibration tool for Tetris/xArm/Realsense.

This version supports:

1. Pick/scatter surface calibration from 9 or 16 points.
   It fits the real surface plane by PCA/SVD, instead of using only P/X/Y.
   Output:
     /tetris/TABLE_SURFACE_PLANE_BASE
     /tetris/PICK_SURFACE_PLANE_BASE

2. White board calibration modes:
   Mode A: 9 points
     (col,row) =
       (0,0),(5,0),(9,0),
       (0,7),(5,7),(9,7),
   ...
   Mode C: 140 points
     all 14x10 bumps, row=0..13, col=0..9

3. Board output:
     /tetris/BOARD_CENTERS_14x10_TABLE
     /tetris/BOARD_BUMP_HEIGHT_MAP_14x10
     /tetris/PLACE_Z_MAP_14x10
...
def choose_pick_surface_sample_count():
    ...
    while True:
        ans = input("散落/抓取平面采样点数选择 9 或 16 [默认 16]: ").strip()
        if ans == "":
            return 16
        try:
            n = int(ans)
...
def main():
    ...
    table_surface_plane_base = {
        "point": [float(x) for x in surface_centroid_base],
        "normal": [float(x) for x in surface_normal_base],
        "residual_rms_m": float(surf_rms),
        "residual_max_abs_m": float(surf_max),
        "sample_count": int(pick_surface_samples),
    }
    pick_plane_point_base = surface_centroid_base + surface_normal_base * block_thickness
    pick_surface_plane_base = {
        "point": [float(x) for x in pick_plane_point_base],
        "normal": [float(x) for x in surface_normal_base],
        "offset_from_table_surface_m": float(block_thickness),
        "meaning": "camera rays should intersect this plane for pick-point XY; table surface plane shifted by block top height",
    }
    ...
    config = {
        "tetris": {
            ...
            "TABLE_SURFACE_PLANE_BASE": table_surface_plane_base,
            "PICK_SURFACE_PLANE_BASE": pick_surface_plane_base,
            "PICK_SURFACE_SAMPLES_BASE": [[float(v) for v in p] for p in scatter_surface_points],
            "PICK_SURFACE_ORIGIN_USER_BASE": [float(v) for v in P_user],
            "PICK_SURFACE_ORIGIN_PROJECTED_BASE": [float(v) for v in P],
            "PICK_SURFACE_XDIR_USER_BASE": [float(v) for v in X_user],
            "PICK_SURFACE_XDIR_PROJECTED_BASE": [float(v) for v in X_proj],
    ...
    print("\n关键结果：")
    print(f"  抓取/散落平面采样点数: {pick_surface_samples}")
    print(f"  抓取/散落平面 residual RMS/max: {surf_rms*1000:.3f} / {surf_max*1000:.3f} mm")
    print(f"  方块顶面/抓取高度 PICK_Z: {pick_z:.6f} m")
    print(f"  PICK_SURFACE_PLANE_BASE.point: {[round(float(v), 6) for v in pick_plane_point_base]}")
    print(f"  PICK_SURFACE_PLANE_BASE.normal: {[round(float(v), 6) for v in surface_normal_base]}")
    ...
    print("  - 控制节点应开启 use_true_pick_plane=true，读取 PICK_SURFACE_PLANE_BASE。")
    print("  - 放置控制继续读取 BOARD_CENTERS_14x10_TABLE 和 PLACE_Z_MAP_14x10。")
