...
//
// 职责：消费 path_planner 发布的 /motion_cmds (tly::MotionPlan)，每个 MotionTask 已是 base 系、
...
private:
    ...
    void planCallback(const tly::MotionPlan::ConstPtr &msg)
    {
        while (!tasks_.empty())
            tasks_.pop();
        ...
        {
            const tly::MotionTask &t = msg->tasks[i];
            ExecTask et;
            et.shape = t.shape;
            et.way = t.way;
