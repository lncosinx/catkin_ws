...
public:
    VisionProcessorNode() : pnh_("~"), it_(nh_), tf_listener(tf_buffer)
    {
        ...
        if (close_kernel > 1)
            cuda_close_filter = cuda::createMorphologyFilter(MORPH_CLOSE, CV_8UC1, Mat::ones(close_kernel, close_kernel, CV_8UC1));
        if (open_kernel > 1)
            cuda_open_filter = cuda::createMorphologyFilter(MORPH_OPEN, CV_8UC1, Mat::ones(open_kernel, open_kernel, CV_8UC1));
    ...
    // 局部 ROI 优化过的 Distance Transform (修复抖动版)
    Point2f distance_transform_center(const vector<Point> &cnt)
    {
        ...
        // 取最大距离 95% 以上的所有像素点
        double threshold_val = max_val * 0.95;
        ...
        for (int y = 0; y < dist.rows; ++y)
        {
            ...
            {
                if (dist.at<float>(y, x) >= threshold_val)
                {
                    sum_x += x;
                    sum_y += y;
                    count++;
        ...
        if (count > 0)
        {
            best_center = Point2f(sum_x / count, sum_y / count);
        }
        else
        {
            // 极端异常情况下的兜底
            best_center = Point2f(bb.width / 2.0f, bb.height / 2.0f);
        }

        return Point2f(best_center.x + bb.x, best_center.y + bb.y);
    }
    ...
    void process_foreground_cuda(const Mat &frame, Mat &small_out, Mat &mask_out, double &scale)
    {
        ...
        if (blur_kernel > 1)
            cuda_blur_filter->apply(d_gray, d_gray);
        ...
        if (close_kernel > 1)
            cuda_close_filter->apply(d_mask, d_mask);
        if (open_kernel > 1)
            cuda_open_filter->apply(d_mask, d_mask);
    ...
    void image_cb(const sensor_msgs::ImageConstPtr &msg)
    {
        ...
        for (auto &t : tracks)
        {
            ...
            {
                if (t.is_stable(stable_min_frames, stable_max_px_std, stable_max_angle_std_deg))
